...
from .forms import MyForm,EducationForm,ExperienceForm,JobForm,InterviewForm
...
class AddFormView(TemplateView):
    ...
    def post(self, *args, **kwargs):
        # test_id = UserForm_uz.objects.get(id=id).test_id
        # education formset
        EducationFormSet = modelformset_factory(Education_uz, form=EducationForm, extra=1)
        ...
        if education_formset.is_valid() and my_form.is_valid() and education_formset.is_valid():
            ...
            return HttpResponse(f"<h2>Рўйхатдан ўтганингиз учун рахмат. Сиз билан тез орада аълоқага чиқамиз </h2>")
        else:
